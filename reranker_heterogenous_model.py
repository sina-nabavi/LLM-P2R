...
def similarity_pipeline(item=None, rank_start=0, rank_end=100, sync_pairs=0, clients_num=5, sentence_transformers_list=None):
    query = item['query']
    ...
    if client_docs_num > 0:
        for i in range(clients_num-1):
            client_messages = create_permutation_instruction(item=item, rank_start=i*client_docs_num, rank_end=(i+1)*client_docs_num)
            messages.append(client_messages)

    client_messages = create_permutation_instruction(item=item, rank_start=(clients_num-1)*client_docs_num, rank_end=rank_end)  
    messages.append(client_messages)

    num_sync_pairs = len(item['hits'][rank_end:rank_end+sync_pairs])
    scale_factors = []
    sync_scores = []
    if num_sync_pairs > 0 and sync_pairs > 0:
        sync_docs = create_permutation_instruction(item=item, rank_start=rank_end, rank_end=rank_end+sync_pairs)
        for i in range(clients_num):
            sync_scores.append(sentence_similarity_batch(sync_docs, query, sentence_transformers_list[i], None, multiGPU=False)[0].tolist())

        # Calculate the reference score as the average of sync_scores for each sync pair
        sync_scores_avg = np.mean(sync_scores, axis=0)

        # Calculate the scaling factors for each client based on the reference score
        for client_scores in sync_scores:
            num = np.dot(client_scores, sync_scores_avg)  # numerator of the scaling factor formula
            denom = np.dot(client_scores, client_scores)  # denominator of the scaling factor formula
            scale_factors.append(num / denom if denom != 0 else 1)  # avoid division by zero
    else:
        scale_factors = [1] * len(messages)
    doc_scores = []
    for i in range(len(messages)):
        ...
        client_doc_scores = sentence_similarity_batch(input_texts, query, sentence_transformers_list[i], pool, multiGPU=True)[0].tolist()
        # Apply scaling factor to each score
        client_doc_scores = [score * scale_factors[i] for score in client_doc_scores]
        doc_scores += client_doc_scores
...
def main():
    ...
    for data in ['dl19','dl20', 'covid', 'nfc', 'touche', 'dbpedia', 'scifact', 'signal', 'news', 'robust04']: 
        ...
        sync_pairs = 1
        rank_results = run_retriever(topics, searcher, qrels, k=100 + sync_pairs)
        new_results = []
        for item in tqdm(rank_results):
            if len(item['hits']) == False:
                continue
            new_item = similarity_pipeline(item, rank_start=0, rank_end=100, sync_pairs=0, clients_num=len(sentence_transformers), sentence_transformers_list=sentence_transformers)
